"""
This script defines several transforms that can be used for multiple images at the same time, e.g., (image, label).
...
While others change image and label simultaneously.
...
class LabelBinarization(object):
    ...
    def __call__(self, imgs):
        ...
        if not self.is_binary:
            # label: one channel (0: background, 1: tumor, 2: lym, 3: stroma) #####
            new_label = np.zeros((label.shape[0], label.shape[1]), dtype=np.uint8)
            new_label[label[:, :, 0] > 255 * 0.5] = 1  # tumor
            new_label[label[:, :, 1] > 255 * 0.5] = 2  # lym
            new_label[label[:, :, 2] > 255 * 0.5] = 3  # stroma
        else:  # binary label
            new_label = np.where(label>255*0.5, 1, 0)

        # import utils
        # utils.show_figures((label, new_label))

        label = Image.fromarray(new_label.astype(np.uint8))
        pics = [imgs[i] for i in range(len(imgs)-1)]
        # pics.append(label_obj)
        pics.append(label)

        return tuple(pics)


class LabelBinarization2(object):
    """
    Binarization for label, computes contours individually
    """
    def __init__(self, is_binary=False):
        self.is_binary = is_binary
    ...
    def __call__(self, imgs):
        ...
        if not self.is_binary:  # label
            # --- label: one channel (0: background, 1: tumor, 2: lymphocyte, 3:stroma, 4: contours)
            new_label = np.zeros((label.shape[0], label.shape[1]), dtype=np.uint8)
            new_label[(label[:, :, 0] > thresh) * (label[:, :, 1] < thresh2) * (label[:, :, 2] < thresh2)] = 1  # tumor inside
            new_label[(label[:, :, 0] < thresh2) * (label[:, :, 1] > thresh) * (label[:, :, 2] < thresh2)] = 2  # lym inside
            new_label[(label[:, :, 0] < thresh2) * (label[:, :, 1] < thresh2) * (label[:, :, 2] > thresh)] = 3  # stroma inside
            contours = morphology.dilation(new_label>0, morphology.disk(2)) & (~(new_label>0))  # l=2
            new_label[contours > 0] = 4  # contours
        else:  # binary label
            new_label = np.where(label>255*0.5, 1, 0)
