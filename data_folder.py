...
def get_imgs_list(dir_list, postfix=None):
    """
    :param dir_list: [img1_dir, img2_dir, ...]
    :param postfix: e.g. ['label.png', 'weight.png',...]
    :return: e.g. [(img1.ext, img1_label.png, img1_weight.png), ...]
    """
    img_list = []
    if len(dir_list) == 0:
        return img_list
    if len(dir_list) != len(postfix) + 1:
    ...
    for img in img_filename_list[0]:
        ...
        for i in range(1, len(img_filename_list)):
            img_name = '{:s}_{:s}'.format(img1_name, postfix[i-1])
...
class DataFolder(data.Dataset):
    def __init__(self, dir_list, postfix, num_channels, data_transform=None, loader=img_loader):
        super(DataFolder, self).__init__()
        if len(dir_list) != len(postfix) + 1:
            raise (RuntimeError('Length of dir_list is different from length of postfix + 1.'))
        if len(dir_list) != len(num_channels):
            raise (RuntimeError('Length of dir_list is different from length of num_channels.'))

        self.img_list = get_imgs_list(dir_list, postfix)
